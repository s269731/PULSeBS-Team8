...
test('Should import enrollments successfully', async () => {
  ...
  const res = await importer.importFile({ tempFilePath: 'importer.test.csv' }, 'enrollments');
  expect(res).toBe(true);
});
...
test('Should import schedules successfully', async () => {
  ...
  const res = await importer.importFile({ tempFilePath: 'importer.test.csv' }, 'schedules');
  expect(res).toBe(true);
});
