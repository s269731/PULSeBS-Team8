  ...
  await Promise.all(subjects.map(async (subject) => {
    ...
    lectures.forEach((lecture) => {
      ...
      dailystatsarray.push({
        date, bookedSeats: lecture.BookedPeople, unoccupiedSeats,
      });
      ...
      if (dayofweekbefore > dayofweek || weeksBetween(d, daybefore) >= 1) {
        ...
        weeklystatsarray.push({
          weekId, weeklyavgbookings, weeklyavgunoccupiedplaces,
        });
        weeklylesson = 0;
        weeklybookedseats = 0;
      ...
      if (month !== actualmonth) {
        ...
        monthlystatsarray.push({
          monthId, monthlyavgbookings, monthlyavgunoccupiedseats,
        });
        monthlylesson = 0;
...
async function computeTeacherPresencesStatistics(teacherId) {
  ...
  let weeklylesson = 0;
  let monthlylesson = 0;
  let weeklypresences = 0;
  let monthlypresences = 0;

  await Promise.all(subjects.map(async (subject) => {
    const lectures = await lecturesDao.getLecturesForPresenceStatisticsBySubjectId(subject.SubjectId);
    let month;
    let dayofweekbefore = 0;
    let dayofweek;
    let daybefore;
    let monthId;
    let monthlyavgpresences;
    let year;
    let actualmonth;
    let weekId;
    let weeklyavgpresences;
    if (subjectId === undefined) subjectId = subject;
    ...
    lectures.forEach((lecture) => {
      ...
      year = dstring.split(' ')[3];
      dailystatsarray.push({
        date, presentPeople: lecture.PresentPeople,
      });
      ...
      if (dayofweekbefore > dayofweek || weeksBetween(d, daybefore) >= 1) {
        // week ended, push the stats for this month in the weeklystatsarray
        weekId = getWeekId(daybefore, dayofweekbefore);
        weeklyavgpresences = weeklypresences / weeklylesson;
        weeklystatsarray.push({
          weekId, weeklyavgpresences,
        });
        weeklylesson = 0;
        weeklypresences = 0;
      }
      ...
      if (month !== actualmonth) {
        ...
        monthlyavgpresences = monthlypresences / monthlylesson;
        monthlystatsarray.push({
          monthId, monthlyavgpresences,
        });
        monthlylesson = 0;
        monthlypresences = 0;
        // month = actualmonth;
      ...
      // eslint-disable-next-line no-plusplus
      monthlylesson++;
      weeklypresences += lecture.PresentPeople;
      monthlypresences += lecture.PresentPeople;
    });
    // eslint-disable-next-line max-len
    // Lectures for that subjectId ended, push subjectId, dailystatsarray, weeklystatsarray, monthlystatsarray in the statsArray
    weekId = getWeekId(daybefore, dayofweekbefore);
    ...
    weeklyavgpresences = weeklypresences / weeklylesson;
    weeklystatsarray.push({ weekId, weeklyavgpresences });
    weeklypresences = 0;
    weeklylesson = 0;
    ...
    monthlyavgpresences = monthlypresences / monthlylesson;
    monthlystatsarray.push({ monthId, monthlyavgpresences });
    monthlylesson = 0;
    monthlypresences = 0;
    statsArray.push({
      subjectId: subject, dailystatsarray, weeklystatsarray, monthlystatsarray,
...
exports.computeTeacherPresencesStatistics = computeTeacherPresencesStatistics;
