...
const array = [
  {
    ...
    Year: 1,
    schedules: [{
      Capacity: 100, Class: 4, Day: 'Mon', Hour: '8:30-11:00', ScheduleId: 1,
    },
    {
  ...
  },
  {
    ...
    Year: 1,
    schedules: [{
      Capacity: 80, Class: 3, Day: 'Wed', Hour: '14:30-17:30', ScheduleId: 3,
    }],
  }];
...
test('Should update the schedule and related lectures for Class and Capacity values', async () => {
  const obj = {
    ScheduleId: 1, SubjectId: 'XX678', Class: 5, Day: 'Mon', Capacity: 120, Hour: '8:30-11:00',
  };
  const lectIds = await scheduleDao.modifySchedule(obj);
  expect(lectIds).toBeTruthy();
});

test('Should update the schedule and related lectures also for Day and Hour values', async () => {
  const obj = {
    ScheduleId: 1, SubjectId: 'XX678', Class: 5, Day: 'Tue', Capacity: 95, Hour: '10:30-12:00',
  };
  const lectIds = await scheduleDao.modifySchedule(obj);
  expect(lectIds).toBeTruthy();
});
